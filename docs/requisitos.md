...
| **S-16 (actualización Fase B, 2026-07-22)** | Siguiendo la convención ya establecida en este documento (documentar la asunción explícitamente y avanzar, en vez de bloquear), se construyó `Insumo.alergenos` con una **heurística DEMO de palabras clave sobre el nombre** del insumo (ver `lib/data/catalog-insumos-alergenos.demo.ts`, con el listado completo de reglas), y un cálculo de alérgenos EFECTIVOS de un producto que se recalcula si se "quita" (simula "sin X") uno de sus insumos (`lib/menu/alergenos.ts`), expuesto hoy como badge estático + un simulador interactivo en la vista de gestión de receta (`components/menu/EditarRecetaModal.tsx`) — la integración EN VIVO dentro del modal de modificadores de toma de pedido (`components/pos/ModificadorModal.tsx`) queda como seguimiento, no se tocó `lib/sales/engine.ts` en este alcance. Esto **NO reemplaza** la necesidad de datos reales verificados por operaciones/nutriología antes de un lanzamiento real: el "Alto" de la columna de impacto sigue vigente exactamente igual mientras la lista sea heurística. | Alto — sigue sin haber datos reales verificados; la heurística es solo para poder demostrar/probar el mecanismo en la DEMO. |
