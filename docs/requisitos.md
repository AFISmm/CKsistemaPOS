...
| **S-02** | **[SUPUESTO]** El hardware objetivo es una estación de mostrador con pantalla táctil, impresora térmica, cajón monedero y terminal de tarjeta, más una pantalla KDS en cocina. Marca/modelo a confirmar por `hardware-perifericos-pos`. | Medio — cambios de drivers/periféricos. |
...
| **S-05** | **[SUPUESTO]** En modo offline, el efectivo opera con normalidad; el cobro con tarjeta depende de la capacidad *store-and-forward* del PSP/terminal. Si el PSP no soporta offline, la tarjeta no está disponible sin conexión y el cajero usa efectivo. A definir con `pagos-pos`. | **Alto** — afecta continuidad de negocio en cortes de internet. |
| **S-06** | **[SUPUESTO]** El impuesto se calcula sobre el subtotal gravable (tras descuentos) y se redondea al centavo por transacción. Método exacto a validar con finanzas por estado. | Medio — implicación fiscal. |
| **S-07** | **[SUPUESTO]** Moneda única USD; sin manejo de múltiples monedas. | Bajo. |
| **S-08** | **[SUPUESTO]** Aunque el MVP es de una tienda, el modelo de datos de impuestos y precios se diseña **por ubicación** para soportar FL vs. TX sin rehacerlo al escalar. Las tasas exactas (estatal + local por jurisdicción) las provee finanzas. | Medio — retrabajo si no se contempla ahora. |
...
1. ¿El PSP/terminal seleccionado soporta pagos con tarjeta offline (*store-and-forward*)? *(bloqueante de S-05)*
2. Tasas de impuesto exactas por jurisdicción en FL y TX y lista de productos exentos/gravados.
3. Reglas fiscales de recibo mínimas exigidas en FL y TX.
4. Umbrales y motivos válidos para descuentos y reembolsos según política interna.
5. Definición horaria oficial de los *dayparts*.
6. Modelo y cantidad de periféricos por tienda piloto.
