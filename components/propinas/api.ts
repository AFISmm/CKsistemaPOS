/**
 * Cliente HTTP del modulo de Reparto de Propinas (Fase B, nomina-pos). Mismo
 * patron y misma regla dura que components/nomina/api.ts / components/empleados/api.ts:
 * solo `fetch` a /api/v1/..., nunca importa lib/db ni lib/propinas en runtime.
 */

import type { Rol } from "@/lib/domain/types";
import type { ResultadoRepartoPropinas, ValidacionPorcentajesReparto } from "@/lib/propinas/reparto";
...
export interface FilaTurnoPropinasApi {
  turnoId: string;
  ubicacionId: string;
  abiertoEn: string;
  cerradoEn: string;
...
export interface ReportePropinasApi {
  desde: string;
  hasta: string;
  validacion: ValidacionPorcentajesReparto;
  turnos: FilaTurnoPropinasApi[];
  totalesPorEmpleado: { empleadoId: string; nombre: string; totalCentavos: number }[];
}

export async function obtenerReportePropinas(filtro: {
  desde: string;
  hasta: string;
  ubicacionId?: string;
}): Promise<ReportePropinasApi> {
  const qs = new URLSearchParams({ desde: filtro.desde, hasta: filtro.hasta });
  if (filtro.ubicacionId) qs.set("ubicacionId", filtro.ubicacionId);
  return solicitar<ReportePropinasApi>(`/propinas?${qs.toString()}`);
}

/** Actualiza el % DEMO de propina configurado para un rol (ver Rol.porcentajePropinaDemo). */
export async function actualizarPorcentajePropinaRol(
  rolId: string,
  porcentajePropinaDemo: number
): Promise<Rol> {
  const { rol } = await solicitar<{ rol: Rol }>("/roles", {
    method: "PATCH",
    body: JSON.stringify({ rolId, porcentajePropinaDemo }),
  });
  return rol;
}
