  ...
  if (!pedido) {
    return (
      <aside className="flex w-full flex-col rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm lg:w-96">
        <p className="text-sm text-neutral-400">Cargando pedido...</p>
      </aside>
    );
  }
  ...
  return (
    <aside className="flex w-full flex-col rounded-2xl border border-neutral-200 bg-white shadow-sm lg:w-96">
      <div className="flex items-center justify-between border-b border-neutral-200 p-4">
        <div>
          <h2 className="text-lg font-bold text-ck-dark">Orden #{pedido.numeroOrden}</h2>
          <p className="text-xs text-neutral-500">{pedido.nombreCliente || "Cliente mostrador"}</p>
        </div>
        {yaEnviado && (
              ...
              <li key={linea.id} className="rounded-lg border border-neutral-100 p-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    {linea.notas && (
                      <p className="mt-1 text-xs italic text-neutral-400">Nota: {linea.notas}</p>
                    )}
                    {yaEnviado && (
      ...
      <div className="space-y-2 p-4 pt-0">
        ...
        >
          Cobrar {formatearDinero(pedido.total)}
        </button>
      </div>
    </aside>
  );
